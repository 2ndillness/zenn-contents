...
最近`Obsidian`を使い始めたので`Zenn`の記事を`Obsidian`で書きたいと思い導入しました。
...
### Zenn.devでの画像へのパス
プロジェクト(リポジトリ)のルート直下に`articles`,`books`,`images`というディレクトリを作成して記事は`articles`配下に、画像は`images`配下に配置しなければいけません。
```
Vault/
  ├── attachments/
  ├── bases/
...
![](../images/スクリーンショット%202026-03-03%20222655.png)
#### しかし、この方法にも問題が
...
ところが、`Vault`を`Zenn`コンテンツに限定してしまうと外部ファイルの閲覧が難しくなってしまいます。メモやwebクリップ等は`Zenn`の記事を書くための専用のファイルという訳では無いので`Zenn`のリポジトリの中には置きたくない。
...
とはいえ、記事を書く時にブラウザやコード用のテキストエディタを別で立ち上げている事は普通にあると思うので、そこまでデメリットとは言えないかも知れません。
### その他の方法
...
色々と調べているうちに`GitHub Actions`でなんとかなりそうだと言う事が分かりました。
## GitHub Actionsで解決する
`Vault`のルートをリポジトリのルートとしない場合はこの方法が良いのではないでしょうか？
この方法は`Obsidian`に限らず、`VS Code`等でも有効な手段だと思います。
...
:::message
デプロイ用のブランチにプッシュしているのは、パスの書き換えが完了する前に非同期にデプロイされたら困るので、確実にパスが書き換えられた後の状態をデプロイしたいからです。
...
### しかし、この方法にも欠点が
ローカルで相対パスを使用するという事は`npx zenn preview`での画像は表示されません。
`Zenn`での表示チェックは非公開で投稿して確認するといった方法になってしまいます。
## という訳でプラグインを作る事にしました
どうしても`Obsidian`で快適に`Zenn`の記事を書いてみたいというのが諦めきれなくて自作の`Obsidian`プラグインを現在作っています。(とはいえほぼバイブコーディングです)

元々`Obsidian`では`npx zenn preview`を利用するためには少々手間がかかるのです。
1. コマンドを実行するためにコミニティプラグインの`Terminal`を起動します。
2. `npx zenn preview`のコマンドを実行します
3. `Obsidian`内蔵ブラウザを立ち上げます。
4. ローカルサーバー(デフォルト`http://localhost:8000/`)へ接続します。

なので、markdownの仕上がりプレビューには`npx zenn preview`を利用せずに`Obsidian`のレンダリングで`Zenn`風のスタイルを再現してしまおうという趣旨のプラグインを目指しています。