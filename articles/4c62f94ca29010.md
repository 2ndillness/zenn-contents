...
予てより気になっていた`Zenn Connect`(GitHub連携)を導入しました。
...
```
:::message
パス変換スクリプトで`glob`ランタイムを使用するのでローカルで`glob`ランタイムをインストールし`package.json`及び`package-lock.json`に依存関係が記載された状態にしておきます。
```bash
npm install glob
```
:::
    ...
    steps:
      ...
      - name: Commit converted files to zenn-deploy
        run: |
          ...
          # 新しいブランチをチェックアウト（存在しなければ作成）
          git checkout -B zenn-deploy

          # 変換済みファイルを zenn-deploy に反映
          git add .
          git commit -m "chore: convert image paths for Zenn" || echo "No changes"
...
## Zennでの設定
`Zenn`へログインした状態でアカウントのアイコン→`GitHub`からのデプロイ→リポジトリ設定タブに入ります。
今回は既に連携したリポジトリの設定を変えるので「設定を変更」ボタンを押して新しいブランチ名という所に新しく作成されたデプロイ用のブランチを指定します。
...
まだまだバグが多くて完成はいつになるやらですが...少しずつコツコツと作っています。
